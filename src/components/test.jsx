...
const solveProblem = (c_hiryou, cost, c_yasai_W, c_yasai_K, c_yasai_N, c_yasai_P, c_hiryo_list_K, c_hiryo_list_N, c_hiryo_list_P) => {
  let model = {
    optimize: 'cost',
    opType: 'min',
    constraints: {},
  ...
  if (result.feasible) {
    ...
    console.log('選んだ肥料の成分が偏っています\n肥料の種類を増やしてください');
    console.log('選ぶのが手間でしたら\nすべての肥料を選択してください\nその中から必要なものを自動で抜粋します');
  }
...
async function main() {
  ...
  const hiryou_df = await loadCSV('hiryou0313.csv');
  const yasai_df = await loadCSV('yasai0228.csv');

  // 選択された野菜と肥料
  const c_yasai = 'トマト';
  const c_hiryou = ['牛ふん堆肥','発酵鶏ふん','豚ふん堆肥','油かす','骨粉',"もみ殻"];
  ...
  // ここで問題を解く関数を呼び出す
  const result = solveProblem(c_hiryou, cost, c_yasai_W, c_yasai_K, c_yasai_N, c_yasai_P, c_hiryo_list_K, c_hiryo_list_N, c_hiryo_list_P);
